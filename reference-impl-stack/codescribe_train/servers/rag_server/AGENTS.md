...
Exposes `find_similar_issues`, `get_pr_diff`, `search_commits`, and
`search_docs` as MCP tools over stdio (four tools, per the design spec).
...
- All tools are READ-ONLY. No file writes, no DB writes, no network calls beyond the local SQLite store.
