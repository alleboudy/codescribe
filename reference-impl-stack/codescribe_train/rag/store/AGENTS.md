...
- All writes go through `codescribe_train/rag/store/writer.py`'s upsert API. Never bypass with raw SQL elsewhere.
- All reads in the retrieval path go through `codescribe_train/rag/store/retrieve.py`. Bypassing fragments query semantics.
- `sqlite-vec` extension is loaded explicitly via `conn.enable_load_extension(True); conn.load_extension("vec0")` — never relies on it being globally available.
- Diff text is GZIP-compressed in `pulls.diff_text` and `commits.diff_text`. Decompression happens at read time in `retrieve.py`.
...
- Do NOT use Python's unsafe binary serialisation modules for any stored field. JSON + gzip only; never `marshal`/`pickle` for stored fields.
