...
7B. Now you have **several** laptops — a fleet of mobile
...
Budget, 4× 8 GB laptop GPUs on 1 Gbps, sweep of 16 configs (~3K-sample dataset):
...
3. [`11-hardware.md`](11-hardware.md) — VRAM math (why `seq_len=1024` on 8 GB), the
   compute-capability map (the fleet GPU is **sm_89**), and laptop thermal/power.
...
## The hardware: 8 GB Ada-class laptops

| Spec | Value | Why it matters |
|---|---|---|
| Architecture | Ada Lovelace, **sm_89** | Modern enough for Flash-Attention 2; CUDA 12.x is clean. llama.cpp build flag: `-DCMAKE_CUDA_ARCHITECTURES=89` (NOT `120`/Blackwell, NOT `75`/Turing). |
...
### Per-laptop checklist (do once, sanity-check before every long run)
...
2. **The vendor power utility → maximum-performance** power plan on AC.
...
1. **Drivers + compute cap:** `nvidia-smi --query-gpu=name,compute_cap --format=csv,noheader`
   → expect `... <fleet GPU> ..., 8.9`. Rebuild llama.cpp with `-DCMAKE_CUDA_ARCHITECTURES=89`
...
- **One worker consistently slower** → almost always thermal. Re-run the per-laptop
  checklist.
